...
def examine_strategies(key, time_frame, data):
    ...
    TBC = tbc.three_black_crows(key, price_action, time_frame)
    if TBC:
        strategyList.append({id.three_black_crow: TBC})
    DTDB = dtdb.double_top_double_bottom(key, price_action, time_frame)
    if DTDB:
        if DTDB[id.name] == id.double_top:
            strategyList.append({id.double_top:DTDB})
        else:
            strategyList.append({id.double_bottom:DTDB})
    TWS = tws.three_white_soldiers(key, price_action, time_frame)
    if TWS:
        strategyList.append({id.three_white_soldiers: TWS})
    BAB = bab.bullish_abandoned_baby(key, price_action, time_frame)
    if BAB:
